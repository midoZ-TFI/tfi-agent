...
import json, logging, os, base64 as b64
...
class SocialAgent:
    ...
    def run(self):
        ...
        post = {"type": tpl["type"], "content": tpl["content"], "hashtags": tpl["hashtags"], "slug": "tfi-weekly-post"}
        ...
        logger.info("Done - %d generated, %d published", len(self.posts_generated), published)
    ...
    def _save_post(self, post):
        ...
        fp = self.posts_dir / (ds + "-tfi-weekly-post.md")
    ...
    def _publish_post(self, post):
        ...
        try:
            ...
            h = {"Authorization": "Bearer " + token, "Content-Type": "application/json", "LinkedIn-Version": "202401", "X-Restli-Protocol-Version": "2.0.0"}
            ...
            if r.status_code in (200, 201):
                ...
                return "published"
            logger.error("Error %d: %s", r.status_code, r.text[:300])
            return None
        except Exception as e:
